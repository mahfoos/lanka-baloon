      ...
      {gallery.map(({ image: name, alt }) => {
        ...
        return (
          <figure
            ...
            className="group relative h-64 shrink-0 snap-start overflow-hidden rounded-2xl bg-mist sm:h-80 lg:h-[26rem]"
          >
            <Pic name={name} alt={alt} fill sizes="(min-width: 1024px) 40vw, 70vw" className="photo-zoom object-cover" />
          </figure>
        );
      })}
    </ScrollRail>
