      ...
      {rates.map((r, i) => {
        ...
        return (
          <Reveal
            ...
            as="article"
            delay={i * 90}
            className={`flex flex-col rounded-3xl p-7 transition-shadow duration-300 hover:shadow-xl ${featured ? "bg-ink text-white" : "border border-line bg-white"}`}
          >
            ...
            <ul className={`mt-6 space-y-2.5 text-[15px] ${featured ? "text-white/85" : "text-ink-soft"}`}>
              {r.includes.map((item) => (
                <li key={item} className="flex gap-3">
                  <svg className="mt-1 shrink-0 text-dawn" width="14" height="14" viewBox="0 0 14 14" aria-hidden>
